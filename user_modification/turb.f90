...
!
! HUGO JACQUET
USE MODD_BLANK_n, ONLY: TDUMMY2,TDUMMY16 ! store LM,sv2_hvar in OUT files, Hugo Jacquet 21/07/23
...
!
REAL, DIMENSION(D%NIJT,D%NKT) :: ZFLXZ ! Hugo Jacquet: to extract subgrid fluxes related to passive tracers
REAL, DIMENSION(D%NIT,D%NJT,D%NKT,KSV) :: ZWORK3
REAL, DIMENSION(D%NIT,D%NJT,D%NKT) :: ZWORK_PDXX,ZWORK_PDYY,ZWORK_PDZZ,ZWORK_PDZX,ZWORK_PDZY
...
! HUGO JACQUET-----
! NOTES:
! * 1.2 is ZCSVD in mode_turb_hor_sv_corr.f90
! * We can use zwork1 and zwork2 as we area at the end of the turb routine
! Not finished, needs more work, especially to compute stability functions here
!	(and not in mode_turb_ver_sv_corr.f90) ...
!CALL MY_UNPACK(PDXX,ZWORK_PDXX)
!CALL MY_UNPACK(PDYY,ZWORK_PDYY)
!CALL MY_UNPACK(PDZZ,ZWORK_PDZZ)
!CALL MY_UNPACK(PDZX,ZWORK_PDZX)
!CALL MY_UNPACK(PDZY,ZWORK_PDZY)

!DO JSV=NSV_CSBEG,NSV_CSEND

!	! horizontal contribution of sv2 (mode_turb_hor_sv_corr.f90)
!	CALL MY_UNPACK(PSVT(:,:,JSV),ZWORK3(:,:,:,JSV))
!	CALL MY_UNPACK(ZLM*ZLEPS,TDUMMY16(:,:,:,JSV))
!	TDUMMY16(:,:,:,JSV) = TURBN%XCHF / 1.2 * TDUMMY16(:,:,:,JSV) *   		&
!		 (  GX_M_M(ZWORK3(:,:,:,JSV),ZWORK_PDXX,ZWORK_PDZZ,ZWORK_PDZX)**2       &
!		  + GY_M_M(ZWORK3(:,:,:,JSV),ZWORK_PDYY,ZWORK_PDZZ,ZWORK_PDZY)**2 )    
!		        
!	!vertical contribution of sv2 (mode_turb_ver_sv_corr.f90)
!	CALL GZ_M_W_PHY(D,PSVT(:,:,JSV),PDZZ,ZWORK1)
!	CALL MZF_PHY(D,ZFLXZ,ZWORK2)
!	!$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
!	ZFLXZ(:,:) =  PPSI_SV(:,:,JSV)*ZWORK1(:,:)**2
!	ZFLXZ(:,:) = TURBN%XCHF / 1.2 * ZLM(:,:) * PLEPS(:,:) * ZWORK2(:,:)
!	!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
!	CALL MY_UNPACK(ZFLXZ,TDUMMY17(:,:,:,JSV))
!END DO   

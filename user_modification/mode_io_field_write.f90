...
USE MODD_BLANK_n,      ONLY: TDUMMY1, TDUMMY2, TDUMMY3,TDUMMY4,TDUMMY5,TDUMMY6,&
			...
			TDUMMY13,TDUMMY14,TDUMMY15,TDUMMY16,TDUMMY17,TDUMMY18,&
			TDUMMY19,TDUMMY20,TDUMMY21
USE MODD_NSV, 	       ONLY : NSV_CSBEG,NSV_CSEND
USE MODD_FIELD_n,      ONLY: XSVT
!USE MODD_PARAMETERS,   ONLY: JPVEXT,JPHEXT
!USE MODD_DIM_n,        ONLY: NKMAX,NIMAX_ll,NJMAX_ll
!
IMPLICIT NONE
!
TYPE(TOUTBAK),    INTENT(IN)  :: TPOUTPUT !Output structure
...
!
!
! Note : As of the 19th of june 2024 :
!	* If a flux is only available through diagnostic (like in NAM_LES), then
!		it is very hard to extract the flux
!		Liste of affected fluxes:
!			rv2,Thl2,sv2,wsv,vsv,usv, (maybe more)
!		As long as online diagnostic will not be available in MNH
!		and that turbulence is treated the same for all variable,
!		some fluxes will not be extracted and would require a lot of work.
!
!	* If there is 4 passive tracers (default+1), then no error rises.
...
!
IF (.FALSE.) THEN
! To use TRUE would require more work to find where thoses fluxes are computed.
!	for each flux, a starting point is indicated.
	! --------
	! TRACER 1
	! --------
	!
	! Look at mode_turb_ver_sv_flux.f90
	TZFIELD = TFIELDMETADATA(                                           &
	   CMNHNAME   = 'WSV_FLX_SVT001',                                   &
	   CSTDNAME   = '',                                                 &
	   CLONGNAME  = 'WSV_FLX_SVT001',                                   &
	   CUNITS     = 'kg kg-1 m s-1',                                    &
	   CDIR       = 'XY',                                               &
	   CCOMMENT   = 'X_Y_Z_WSV_FLX_SVT001',                             &
	   NGRID      = 4,                                                  &
	   NTYPE      = TYPEREAL,                                           &
	   NDIMS      = 3,                                                  &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY13(:,:,:,1))
	!
	! look at mode_turb_hor_sv_flux.f90
	TZFIELD = TFIELDMETADATA(                                           &
	   CMNHNAME   = 'USV_FLX_SVT001',                                   &
	   CSTDNAME   = '',                                                 &
	   CLONGNAME  = 'USV_FLX_SVT001',                                   &
	   CUNITS     = 'kg kg-1 m s-1',                                    &
	   CDIR       = 'XY',                                               &
	   CCOMMENT   = 'X_Y_Z_USV_FLX_SVT001',                             &
	   NGRID      = 2,                                                  &
	   NTYPE      = TYPEREAL,                                           &
	   NDIMS      = 3,                                                  &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY14(:,:,:,1))
	!
	! look at mode_turb_hor_sv_flux.f90
	TZFIELD = TFIELDMETADATA(                                           &
	   CMNHNAME   = 'VSV_FLX_SVT001',                                   &
	   CSTDNAME   = '',                                                 &
	   CLONGNAME  = 'VSV_FLX_SVT001',                                   &
	   CUNITS     = 'kg kg-1 m s-1',                                    &
	   CDIR       = 'XY',                                               &
	   CCOMMENT   = 'X_Y_Z_VSV_FLX_SVT001',                             &
	   NGRID      = 3,                                                  &
	   NTYPE      = TYPEREAL,                                           &
	   NDIMS      = 3,                                                  &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY15(:,:,:,1))
	!
	! look at mode_turb_hor_sv_corr.f90 for horizontal part
	!         mode_turb_ver_sv_corr.f90 for vertical part
	! Could be combined in turb.f90
	TZFIELD = TFIELDMETADATA(                                                &
	   CMNHNAME   = 'SV_VAR_SVT001',                                         &
	   CSTDNAME   = '',                                                      &
	   CLONGNAME  = 'SV2_VAR_SVT001',                                        &
	   CUNITS     = 'kg2 kg-2',                                              &
	   CDIR       = 'XY',                                           	 &
	   CCOMMENT   = 'X_Y_Z_SV2_VAR_SVT001', 				 &
	   NGRID      = 1,                                                	 &
	   NTYPE      = TYPEREAL,                                          	 &
	   NDIMS      = 3,                                                 	 &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD, TDUMMY16(:,:,:,1) + TDUMMY17(:,:,:,1) ) 
	!
	!
	! --------
	! TRACER 2
	! --------
	IF (.FALSE.) THEN ! no need if no cloud
		TZFIELD = TFIELDMETADATA(                                           &
		   CMNHNAME   = 'SVT002',                                           &
		   CSTDNAME   = '',                                                 &
		   CLONGNAME  = 'SVT002',                                           &
		   CUNITS     = 'kg kg-1',                                          &
		   CDIR       = 'XY',                                               &
		   CCOMMENT   = 'X_Y_Z_SVT002',                                     &
		   NGRID      = 1,                                                  &
		   NTYPE      = TYPEREAL,                                           &
		   NDIMS      = 3,                                                  &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,XSVT(:,:,:,NSV_CSBEG+1))
		!
		TZFIELD = TFIELDMETADATA(                                           &
		   CMNHNAME   = 'WSV_FLX_SVT002',                                   &
		   CSTDNAME   = '',                                                 &
		   CLONGNAME  = 'WSV_FLX_SVT002',                                   &
		   CUNITS     = 'kg kg-1 m s-1',                                    &
		   CDIR       = 'XY',                                               &
		   CCOMMENT   = 'X_Y_Z_WSV_FLX_SVT002',                             &
		   NGRID      = 4,                                                  &
		   NTYPE      = TYPEREAL,                                           &
		   NDIMS      = 3,                                                  &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY13(:,:,:,2))
		!
		TZFIELD = TFIELDMETADATA(                                           &
		   CMNHNAME   = 'USV_FLX_SVT002',                                   &
		   CSTDNAME   = '',                                                 &
		   CLONGNAME  = 'USV_FLX_SVT002',                                   &
		   CUNITS     = 'kg kg-1 m s-1',                                    &
		   CDIR       = 'XY',                                               &
		   CCOMMENT   = 'X_Y_Z_USV_FLX_SVT002',                             &
		   NGRID      = 2,                                                  &
		   NTYPE      = TYPEREAL,                                           &
		   NDIMS      = 3,                                                  &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY14(:,:,:,2))
		!
		TZFIELD = TFIELDMETADATA(                                           &
		   CMNHNAME   = 'VSV_FLX_SVT002',                                   &
		   CSTDNAME   = '',                                                 &
		   CLONGNAME  = 'VSV_FLX_SVT002',                                   &
		   CUNITS     = 'kg kg-1 m s-1',                                    &
		   CDIR       = 'XY',                                               &
		   CCOMMENT   = 'X_Y_Z_VSV_FLX_SVT002',                             &
		   NGRID      = 3,                                                  &
		   NTYPE      = TYPEREAL,                                           &
		   NDIMS      = 3,                                                  &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY15(:,:,:,2))
		!
		TZFIELD = TFIELDMETADATA(                                                &
		   CMNHNAME   = 'SV_VAR_SVT002',                                         &
		   CSTDNAME   = '',                                                      &
		   CLONGNAME  = 'SV2_VAR_SVT002',                                        &
		   CUNITS     = 'kg2 kg-2',                                              &
		   CDIR       = 'XY',                                           	 &
		   CCOMMENT   = 'X_Y_Z_SV2_VAR_SVT002', 				 &
		   NGRID      = 1,                                                	 &
		   NTYPE      = TYPEREAL,                                          	 &
		   NDIMS      = 3,                                                 	 &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD, TDUMMY16(:,:,:,2)+TDUMMY17(:,:,:,2))  
	ENDIF
	!
	! --------
	! TRACER 3
	...
	!
	TZFIELD = TFIELDMETADATA(                                           &
	   CMNHNAME   = 'WSV_FLX_SVT003',                                   &
	   CSTDNAME   = '',                                                 &
	   CLONGNAME  = 'WSV_FLX_SVT003',                                   &
	   CUNITS     = 'kg kg-1 m s-1',                                    &
	   CDIR       = 'XY',                                               &
	   CCOMMENT   = 'X_Y_Z_WSV_FLX_SVT003',                             &
	   NGRID      = 4,                                                  &
	   NTYPE      = TYPEREAL,                                           &
	   NDIMS      = 3,                                                  &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY13(:,:,:,3))
	!
	TZFIELD = TFIELDMETADATA(                                           &
	   CMNHNAME   = 'USV_FLX_SVT003',                                   &
	   CSTDNAME   = '',                                                 &
	   CLONGNAME  = 'USV_FLX_SVT003',                                   &
	   CUNITS     = 'kg kg-1 m s-1',                                    &
	   CDIR       = 'XY',                                               &
	   CCOMMENT   = 'X_Y_Z_USV_FLX_SVT003',                             &
	   NGRID      = 2,                                                  &
	   NTYPE      = TYPEREAL,                                           &
	   NDIMS      = 3,                                                  &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY14(:,:,:,3))
	!
	TZFIELD = TFIELDMETADATA(                                           &
	   CMNHNAME   = 'VSV_FLX_SVT003',                                   &
	   CSTDNAME   = '',                                                 &
	   CLONGNAME  = 'VSV_FLX_SVT003',                                   &
	   CUNITS     = 'kg kg-1 m s-1',                                    &
	   CDIR       = 'XY',                                               &
	   CCOMMENT   = 'X_Y_Z_VSV_FLX_SVT003',                             &
	   NGRID      = 3,                                                  &
	   NTYPE      = TYPEREAL,                                           &
	   NDIMS      = 3,                                                  &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY15(:,:,:,3))
	!
	TZFIELD = TFIELDMETADATA(                                                &
	   CMNHNAME   = 'SV_VAR_SVT003',                                         &
	   CSTDNAME   = '',                                                      &
	   CLONGNAME  = 'SV2_VAR_SVT003',                                        &
	   CUNITS     = 'kg2 kg-2',                                              &
	   CDIR       = 'XY',                                           	 &
	   CCOMMENT   = 'X_Y_Z_SV2_VAR_SVT003', 				 &
	   NGRID      = 1,                                                	 &
	   NTYPE      = TYPEREAL,                                          	 &
	   NDIMS      = 3,                                                 	 &
	   LTIMEDEP   = .TRUE.                                              )   
	CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY16(:,:,:,3)+TDUMMY17(:,:,:,3))
	!
	!
	! --------
	! TRACER 4
	...
	if (.FALSE.) THEN
		!
		TZFIELD = TFIELDMETADATA(                                           &
		   CMNHNAME   = 'WSV_FLX_SVT004',                                   &
		   CSTDNAME   = '',                                                 &
		   CLONGNAME  = 'WSV_FLX_SVT004',                                   &
		   CUNITS     = 'kg kg-1 m s-1',                                    &
		   CDIR       = 'XY',                                               &
		   CCOMMENT   = 'X_Y_Z_WSV_FLX_SVT004',                             &
		   NGRID      = 4,                                                  &
		   NTYPE      = TYPEREAL,                                           &
		   NDIMS      = 3,                                                  &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY13(:,:,:,4))
		!
		TZFIELD = TFIELDMETADATA(                                           &
		   CMNHNAME   = 'USV_FLX_SVT004',                                   &
		   CSTDNAME   = '',                                                 &
		   CLONGNAME  = 'USV_FLX_SVT004',                                   &
		   CUNITS     = 'kg kg-1 m s-1',                                    &
		   CDIR       = 'XY',                                               &
		   CCOMMENT   = 'X_Y_Z_USV_FLX_SVT004',                             &
		   NGRID      = 2,                                                  &
		   NTYPE      = TYPEREAL,                                           &
		   NDIMS      = 3,                                                  &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY14(:,:,:,4))
		!
		TZFIELD = TFIELDMETADATA(                                           &
		   CMNHNAME   = 'VSV_FLX_SVT004',                                   &
		   CSTDNAME   = '',                                                 &
		   CLONGNAME  = 'VSV_FLX_SVT004',                                   &
		   CUNITS     = 'kg kg-1 m s-1',                                    &
		   CDIR       = 'XY',                                               &
		   CCOMMENT   = 'X_Y_Z_VSV_FLX_SVT004',                             &
		   NGRID      = 3,                                                  &
		   NTYPE      = TYPEREAL,                                           &
		   NDIMS      = 3,                                                  &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY15(:,:,:,4))
		!
		TZFIELD = TFIELDMETADATA(                                                &
		   CMNHNAME   = 'SV_VAR_SVT004',                                         &
		   CSTDNAME   = '',                                                      &
		   CLONGNAME  = 'SV2_VAR_SVT004',                                        &
		   CUNITS     = 'kg2 kg-2',                                              &
		   CDIR       = 'XY',                                           	 &
		   CCOMMENT   = 'X_Y_Z_SV2_VAR_SVT004', 				 &
		   NGRID      = 1,                                                	 &
		   NTYPE      = TYPEREAL,                                          	 &
		   NDIMS      = 3,                                                 	 &
		   LTIMEDEP   = .TRUE.                                              )   
		CALL IO_Field_write(TPOUTPUT%TFILE,TZFIELD,TDUMMY16(:,:,:,4)+TDUMMY17(:,:,:,4))
	!
	ENDIF
ENDIF

...
- `save_S_n_SAR` in `mod_spatial_stats.py`: This function saves the values of the Nth stucture function for SAR data. This is done for each boxe from 'd_boxes'. This is parallelized with dask, but it can also be run in serial if needed.
- `save_S_n_LES` in `mod_spatial_stats.py`: This function saves the values of the Nth stucture function for LES data. This is done for each boxe from 'd_boxes'. This is parallelized with dask, but it can also be run in serial if needed.
